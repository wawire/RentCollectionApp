...
const API_BASE_URL =
  process.env.NEXT_PUBLIC_API_URL || 'https://localhost:54060/api';
...
export const publicListingService = {
  ...
  async getPublicProperties(): Promise<PublicPropertyListing[]> {
    const response = await publicApiClient.get<PublicPropertyListing[]>(
      '/public/properties'
    );
  ...
  async getVacantUnits(): Promise<PublicUnitListing[]> {
    const response = await publicApiClient.get<PublicUnitListing[]>(
      '/public/units/vacant'
    );
    return response.data;
  },
   ...
   */
  async getVacantUnitsByProperty(
    propertyId: number
  ): Promise<PublicUnitListing[]> {
    const response = await publicApiClient.get<PublicUnitListing[]>(
      `/public/properties/${propertyId}/units/vacant`
    );
    return response.data;
  },
  ...
  async submitApplication(
    ...
    const response = await publicApiClient.post<TenantApplicationResponse>(
      '/public/applications',
      application
    );
    return response.data;
